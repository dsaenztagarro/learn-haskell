...
data type itself, which is plain `Foo`.
...
compatible function. These two things are each of type `Foo` and can be put in a
list.
...
defines a record selector function only for fields whose type does not mention
the existentially-quantified variables. (This example used an underscore in the
fields for which record selectors will not be defined, but that is only
programming style; GHC ignores them.)
